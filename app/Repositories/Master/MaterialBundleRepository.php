...
class MaterialBundleRepository extends RepositoryAbs
{
    public function getAll($is_minified)
    {
        try {
            $query = MaterialBundle::query();
            ...
            $material_bundle = $query->paginate($perPage, ['*'], 'page', $this->request->page);

            if ($this->request->filled('search') && $material_bundle->isEmpty()) {
                $material_bundle = $query->paginate($perPage, ['*'], 'page', $this->request->page);
            }

            $result = [
                'data' => $material_bundle->items(),
                'per_page' => $material_bundle->perPage(),
            ];

            $result['paginate'] = [
                'current_page' => $material_bundle->currentPage(),
                'last_page' => $material_bundle->lastPage(),
                'total' => $material_bundle->total(),
            ];
    ...
    public function createMaterialBundleFormExcel()
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                foreach ($data as $row) {
                    ...
                    foreach ($customer_group_names as $customer_group_name) {
                        ...
                        $existing_bundle = MaterialBundle::where('sap_code', $sap_code)
                            ->where('customer_group_id', $customer_group->id)
                            ->first();

                        if ($existing_bundle) {
                            // Bộ mã đã tồn tại, cập nhật thông tin
                            $existing_bundle->name = $name;
                            $existing_bundle->bar_code = $bar_code;
                            $existing_bundle->is_active = ($is_active !== null) ? 1 : 0; // Cập nhật trường is_active
                            $existing_bundle->save();

                            $result[] = $existing_bundle;
                        } else {
                            // Tạo mới bộ mã
                            $new_material_bundle = MaterialBundle::create([
    ...
    public function exportToExcel()
    {
        try {
            ...
            $material_bundle = $query->orderBy('id', 'desc')->get();
            ...
            $row = 2;
            foreach ($material_bundle as $bundle) {
                $sheet->setCellValue('A' . $row, $bundle->customer_group->name);
                $sheet->setCellValue('B' . $row, $bundle->sap_code);
                $sheet->setCellValue('C' . $row, $bundle->name);
                $sheet->setCellValue('D' . $row, $bundle->bar_code);
            ...
            $filename = 'material_bundles.xlsx';
        ...
        try {
            $validator = Validator::make(
                ...
                [
                    ...
                    'sap_code' => 'required|unique:material_bundles,sap_code,NULL,id,customer_group_id,' . $this->data['customer_group_id'],
                    // 'bar_code' => 'string',
                    'name' => 'required',
                    'is_active' => 'in:0,1',
            ...
            if ($validator->fails()) {
                ...
                $material_bundle = MaterialBundle::create($this->data);
    ...
    {
        try {
            $material_bundle = MaterialBundle::where('id', $id)->firstOrFail();
            ...
            $material_bundle = MaterialBundle::findOrFail($id);

            // Tiếp tục với việc cập nhật dữ liệu
            $material_bundle->update($this->data);
            return $material_bundle;
        } catch (\Exception $exception) {
            $this->message = $exception->getMessage();
            $this->errors = $exception->getTrace();
    ...
    {
        try {
            $material_bundle = MaterialBundle::find($id);
            $material_bundle->delete();
            return $material_bundle;
        } catch (\Exception $exception) {
            $this->message = $exception->getMessage();
            $this->errors = $exception->getTrace();
    ...
    {
        try {
            $deletedItems = MaterialBundle::whereIn('id', $ids)->get(); // Lấy các bản ghi cần xóa

            if ($deletedItems->isNotEmpty()) { // Kiểm tra xem có bản ghi nào để xóa hay không
                $deletedCount = MaterialBundle::whereIn('id', $ids)->delete(); // Xóa bản ghi
