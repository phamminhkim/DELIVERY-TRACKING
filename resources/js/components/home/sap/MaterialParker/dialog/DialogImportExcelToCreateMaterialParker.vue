<template>
	<div
		class="modal fade"
		id="DialogImportExcelToCreateMaterialParker"
		tabindex="-1"
		role="dialog"
	...
	>
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<form @submit.prevent="createNewMaterialParker">
...
<script>
	...
	export default {
		name: 'DialogImportExcelToCreateMaterialParker',
		components: {
			Vue,
			Treeselect,
			TextInputSearch,
		...
		methods: {
			async createNewMaterialParker() {
				try {
					...
					const { data } = await this.api_handler
						...
						.post(
							'/api/master/material-parkers/excel',
							{},
					...
					if (!data.errors) {
						if (Array.isArray(data)) {
							this.material_parkers.push(...data); // Add the new mappings to the end of the list
						}
			...
			closeDialog() {
				this.resetForm();
				this.resetDialog();
				$('#DialogImportExcelToCreateMaterialParker').modal('hide');
			},
