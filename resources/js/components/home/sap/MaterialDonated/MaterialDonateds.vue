						...
						<div class="card-body">
							<div class="form-group row">
								...
								<div class="col-sm-10 mt-1 mb-1">
									<treeselect placeholder="Nhập mã/ tên khuyến mãi.." :multiple="true" required
				...
				<div class="card-body">
					...
					<div class="row">
						<b-table responsive hover striped show-empty :busy="is_loading" :bordered="true"
							...
							:filter="search_pattern" :fields="fields" :items="material_donateds.data"
							...
							<template #cell(action)="data">
								<div class="margin">
									...
									<button class="btn btn-xs mr-1" @click="deleteMaterialDonated(data.item.id)">
					...
					<div class="row">
						...
						<div class="col-md-3">
							<b-pagination v-model="pagination.current_page" :total-rows="material_donateds.data.length"
					...
					<!-- tạo form -->
					<DialogAddUpdateMaterialDonated ref="AddUpdateDialog" :is_editing="is_editing"
						:editing_item="editing_item" :refetchData="fetchOptionsData"></DialogAddUpdateMaterialDonated>
...
export default {
	...
	methods: {
		...
		deletePaginatedItemsFromSelected() {
			...
			this.checkbox.selected.forEach((item, index_selected) => {
				const index = this.material_donateds.data.findIndex((paginatedItem) => paginatedItem.id === item.id);
				if (index > -1) {
					indexs.push(index_selected);
					this.material_donateds.data.splice(index, 1);
				}
			});
		...
		async fetchData() {
			try {
				...
				if (Array.isArray(data)) {
					this.material_donateds.data = data.map();
				}
	...
	computed: {
		...
		paginatedItems() {
			...
			return this.material_donateds.data.slice(start, end);
		},
	},
};
</script>
