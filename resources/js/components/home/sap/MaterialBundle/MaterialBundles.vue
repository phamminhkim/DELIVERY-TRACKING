							...
							<div class="form-group row">
								...
								<div class="col-sm-10 mt-1 mb-1">
									<treeselect placeholder="Nhập khách hàng.." :multiple="true" required
										...
										v-model="form_filter.material_bundle" />
								</div>
							</div>
				...
				<div class="card-body">
					...
					<div class="row">
						<b-table responsive hover striped show-empty :busy="is_loading" :bordered="true"
							...
							:filter="search_pattern" :fields="fields" :items="material_bundles.data"
							...
							<template #empty="scope">
								<h6 class="text-center">
									Không có khuyến mãi Bundle nào để hiển thị
								</h6>
							</template>
							<template #table-busy>
							...
							<template #cell(action)="data">
								<div class="margin">
									...
									<button class="btn btn-xs mr-1" @click="deleteMaterialBundle(data.item.id)">
					...
					<div class="row">
						...
						<div class="col-md-3">
							<b-pagination v-model="pagination.current_page" :total-rows="material_bundles.data.length"
					...
					<!-- tạo form -->
					<DialogAddUpdateMaterialBundle ref="AddUpdateDialog" :is_editing="is_editing"
						:editing_item="editing_item" :refetchData="fetchOptionsData"></DialogAddUpdateMaterialBundle>
					<DialogImportExcelToCreateMaterialBundle :refetchData="fetchOptionsData" />
...
import DialogAddUpdateMaterialBundle from './dialog/DialogAddUpdateMaterialBundle.vue';
import DialogImportExcelToCreateMaterialBundle from './dialog/DialogImportExcelToCreateMaterialBundle.vue';

export default {
	components: {
		Treeselect,
		Vue,
		DialogAddUpdateMaterialBundle,
		DialogImportExcelToCreateMaterialBundle,
	},
	data() {
		return {
	...
	methods: {
		...
		deletePaginatedItemsFromSelected() {
			...
			this.checkbox.selected.forEach((item, index_selected) => {
				const index = this.material_bundles.data.findIndex((paginatedItem) => paginatedItem.id === item.id);
				if (index > -1) {
					indexs.push(index_selected);
					this.material_bundles.data.splice(index, 1);
				}
			});
		...
		async fetchData() {
			try {
				...
				const params = {
					...
					ids: this.form_filter.material_bundle,
					customer_group_ids: this.form_filter.customer_group,
				};
				const response = await this.api_handler.get(this.api_url, { params });
				const { data, paginate } = response.data.material_bundles;

				if (Array.isArray(data)) {
					this.material_bundles.data = data.map();
				}
		...
		async loadOptions({ action, searchQuery, callback }) {
			if (action === ASYNC_SEARCH) {
				...
				const { data } = await this.api_handler.get(
					'api/master/material-bundles/minified',
					params,
				);
		...
		async clearFilter() {
			try {
				...
				this.form_filter.material_bundle = [];
		...
		async deleteMaterialBundle(id) {
			if (confirm('Bạn muốn xoá?')) {
				try {
					let result = await this.api_handler.delete(`${this.api_url}/${id}`);
					if (result.success) {
						if (Array.isArray(result.data)) {
							this.material_bundles.data = result.data;
						}
						this.showMessage('success', 'Xóa thành công', result.message);
		...
		showCreateDialog() {
			...
			this.editing_item = {};
			$('#DialogAddUpdateMaterialBundle').modal('show');
		},
		showEditDialog(item) {
		...
		showExcelDialog() {
			$('#DialogImportExcelToCreateMaterialBundle').modal('show');
		},
		async exportToExcel() {
			try {
				const params = {
				...
				const response = await this.api_handler.get(
					'api/master/material-bundles/exportToExcel',
					params,
					'blob',
				);
				...
				link.href = url;
				link.setAttribute('download', 'Dữ liệu Bundle.xlsx');
	...
	computed: {
		...
		paginatedItems() {
			...
			return this.material_bundles.data.slice(start, end);
		},
	},
};
</script>
