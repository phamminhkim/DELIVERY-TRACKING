<template>
	<div
		class="modal fade"
		id="DialogImportExcelToCreateMaterialBundle"
		tabindex="-1"
		role="dialog"
	...
	>
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<form @submit.prevent="createNewMaterialBundle">
...
<script>
	...
	export default {
		name: 'DialogImportExcelToCreateMaterialBundle',
		components: {
			Vue,
			Treeselect,
			TextInputSearch,
		...
		methods: {
			async createNewMaterialBundle() {
				try {
					...
					const { data } = await this.api_handler
						...
						.post(
							'/api/master/material-bundles/excel',
							{},
					...
					if (!data.errors) {
						if (Array.isArray(data)) {
							this.material_bundles.push(...data); // Add the new mappings to the end of the list
						}
			...
			closeDialog() {
				this.resetForm();
				this.resetDialog();
				$('#DialogImportExcelToCreateMaterialBundle').modal('hide');
			},
