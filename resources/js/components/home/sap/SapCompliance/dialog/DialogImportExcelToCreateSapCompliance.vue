<template>
	<div
		class="modal fade"
		id="DialogImportExcelToCreateSapCompliance"
		tabindex="-1"
		role="dialog"
	...
	>
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<form @submit.prevent="createNewSapCompliance">
...
<script>
	...
	export default {
		name: 'DialogImportExcelToCreateSapCompliance',
		components: {
			Vue,
			Treeselect,
			TextInputSearch,
		...
		methods: {
			async createNewSapCompliance() {
				try {
					...
					const { data } = await this.api_handler
						...
						.post(
							'/api/master/sap-compliances/excel',
							{},
			...
			downloadTemplate() {
				const filename = 'SapCompliance.xlsx';
			...
			closeDialog() {
				this.resetForm();
				this.resetDialog();
				$('#DialogImportExcelToCreateSapCompliance').modal('hide');
			},
