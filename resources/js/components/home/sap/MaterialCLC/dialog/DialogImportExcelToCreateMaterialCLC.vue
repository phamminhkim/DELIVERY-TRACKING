<template>
	<div
		class="modal fade"
		id="DialogImportExcelToCreateMaterialCLC"
		tabindex="-1"
		role="dialog"
	...
	>
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				<form @submit.prevent="createNewMaterialCLC">
...
<script>
	...
	export default {
		name: 'DialogImportExcelToCreateMaterialCLC',
		components: {
			Vue,
			Treeselect,
			TextInputSearch,
		...
		methods: {
			async createNewMaterialCLC() {
				try {
					...
					const { data } = await this.api_handler
						...
						.post(
							'/api/master/material-clcs/excel',
							{},
					...
					if (!data.errors) {
						if (Array.isArray(data)) {
							this.material_clcs.push(...data); // Add the new mappings to the end of the list
						}
			...
			closeDialog() {
				this.resetForm();
				this.resetDialog();
				$('#DialogImportExcelToCreateMaterialCLC').modal('hide');
			},
