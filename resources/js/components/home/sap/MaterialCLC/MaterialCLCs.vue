							...
							<div class="form-group row">
								...
								<div class="col-sm-10 mt-1 mb-1">
									<treeselect placeholder="Nhập khách hàng.." :multiple="true" required
										...
										v-model="form_filter.material_clc" />
								</div>
							</div>
				...
				<div class="card-body">
					...
					<div class="row">
						<b-table responsive hover striped show-empty :busy="is_loading" :bordered="true"
							...
							:filter="search_pattern" :fields="fields" :items="material_clcs.data"
							...
							<template #empty="scope">
								<h6 class="text-center">
									Không có khuyến mãi CLC nào để hiển thị
								</h6>
							</template>
							<template #table-busy>
							...
							<template #cell(action)="data">
								<div class="margin">
									...
									<button class="btn btn-xs mr-1" @click="deleteMaterialCLC(data.item.id)">
					...
					<div class="row">
						...
						<div class="col-md-3">
							<b-pagination v-model="pagination.current_page" :total-rows="material_clcs.data.length"
					...
					<!-- tạo form -->
					<DialogAddUpdateMaterialCLC ref="AddUpdateDialog" :is_editing="is_editing"
						:editing_item="editing_item" :refetchData="fetchOptionsData"></DialogAddUpdateMaterialCLC>
					<DialogImportExcelToCreateMaterialCLC :refetchData="fetchOptionsData" />
...
import DialogAddUpdateMaterialCLC from './dialog/DialogAddUpdateMaterialCLC.vue';
import DialogImportExcelToCreateMaterialCLC from './dialog/DialogImportExcelToCreateMaterialCLC.vue';

export default {
	components: {
		Treeselect,
		Vue,
		DialogAddUpdateMaterialCLC,
		DialogImportExcelToCreateMaterialCLC,
	},
	data() {
		return {
	...
	methods: {
		...
		deletePaginatedItemsFromSelected() {
			...
			this.checkbox.selected.forEach((item, index_selected) => {
				const index = this.material_clcs.data.findIndex((paginatedItem) => paginatedItem.id === item.id);
				if (index > -1) {
					indexs.push(index_selected);
					this.material_clcs.data.splice(index, 1);
				}
			});
		...
		async loadOptions({ action, searchQuery, callback }) {
			if (action === ASYNC_SEARCH) {
				...
				const { data } = await this.api_handler.get(
					'api/master/material-clcs/minified',
					params,
				);
		...
		async deleteMaterialCLC(id) {
			if (confirm('Bạn muốn xoá?')) {
				try {
					let result = await this.api_handler.delete(`${this.api_url}/${id}`);
					if (result.success) {
						if (Array.isArray(result.data)) {
							this.material_clcs.data = result.data;
						}
						this.showMessage('success', 'Xóa thành công', result.message);
		...
		showCreateDialog() {
			...
			this.editing_item = {};
			$('#DialogAddUpdateMaterialCLC').modal('show');
		},
		showEditDialog(item) {
		...
		showExcelDialog() {
			$('#DialogImportExcelToCreateMaterialCLC').modal('show');
		},
		async exportToExcel() {
			try {
				const params = {
				...
				const response = await this.api_handler.get(
					'api/master/material-clcs/exportToExcel',
					params,
					'blob',
				);
				...
				link.href = url;
				link.setAttribute('download', 'Dữ liệu clc.xlsx');
	...
	computed: {
		...
		paginatedItems() {
			...
			return this.material_clcs.data.slice(start, end);
		},
	},
};
</script>
