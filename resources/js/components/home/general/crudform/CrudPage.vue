<template>
	...
	<div class="container-fluid">
		...
		<div class="row">
			...
			<div class="col-md-3">
				<div class="input-group input-group-sm mt-1 mb-1">
					<input class="form-control -control-navbar" v-model="search_pattern" :placeholder="search_placeholder"
						v-on:keyup.enter="
					...
					<div class="input-group-append">
						<button class="btn btn-default" style="background: #1b1a1a; color: white"
							@click="page_structure.table.fulltextsearch ? onSearching() : undefined">
							<i class="fas fa-search"></i>
						</button>
						<button class="btn btn-danger" v-if="page_structure.table.fulltextsearch" @click="clearSearching()">
		...
		<div class="row">
			<b-table responsive hover striped show-empty :busy="is_loading" :bordered="true" :fields="fields"
			...
			>
				...
				<template #cell(index)="data">
					{{ data.index +( pagination.current_page - 1) * per_page + 1 }}
				</template>
				...
				<template #cell(action)="data">
					<div class="margin">
						...
						<button class="btn btn-xs mr-1" @click="deleteItem(data.item[primary_key])">
			...
			<div class="col-md-12">
				<div class="form-group row">
					<label class="col-form-label-sm col-md-1" style="text-align: left" for="">Per
						page:</label>
					<div class="col-md-3">
						<b-form-select size="sm" v-model="per_page" :options="page_options" @change="fetchData()">
						</b-form-select>
					</div>
					<label class="col-form-label-sm col-md-1" style="text-align: left" for=""></label>
					<div class="col-md-3">
						<b-pagination size="sm" :total-rows="pagination.total" :per-page="per_page" v-model="page"
							@change="fetchData()" />
					</div>
				</div>
			</div>
		</div>
		<CrudDialog :id="dialog_name" :is_editing="is_editing" :editing_item="editing_item" :refetchData="fetchData"
			:form_structure="page_structure.form" :page_url_create="page_structure.api_url"
			:page_url_update="page_structure.api_url" :dialog_name="dialog_name" :primary_key="primary_key"
			@itemCreated="itemCreated" @itemUpdated="itemUpdated"></CrudDialog>
		<!-- end container -->
	</div>
</template>
	...
	methods: {
		async fetchData() {
			try {
				...
				let result = await this.api_handler.get(this.api_item_data + '?' + params.toString());
				this.items = result.data.sap_material_mappings;
				this.pagination.current_page = result.data.paginate.current_page;
				this.pagination.last_page = result.data.paginate.last_page;
				this.pagination.total = result.data.paginate.total;
				this.rendered_items = result.data.sap_material_mappings;
